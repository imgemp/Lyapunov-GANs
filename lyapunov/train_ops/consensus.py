...
class Consensus(Map):
    ...
    def map(self, aux_d, aux_g, d_error_grad, g_error_grad, d_aux, g_aux, V, norm_d, norm_g):
        ...
        #     # embed()
        #     norm_d_grad = torch.autograd.grad(norm, Dparams_reachable, create_graph=True) + zero_grads
        # else:
        #     norm_d_grad = torch.autograd.grad(norm, self.m.D.parameters(), create_graph=True)
